...
export default function AgentConsolePage() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const result = await fetchJson<{ ok: boolean; response?: string; sessionId?: string; model?: string }>(
        '/api/subagents/spawn',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: selectedAgent, task }),
        }
      );

      if (result.sessionId) setActiveSession(result.sessionId);
      setTurns((current) => [
        ...current,
        {
          id: `a-${Date.now()}`,
          ...
          meta: `${agents[selectedAgent]?.name || selectedAgent} • ${result.model || selectedModel}`,
        },
      ]);
